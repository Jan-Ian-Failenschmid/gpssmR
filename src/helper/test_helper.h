...
inline void set_r_seed(double seed)
{
...
inline bool compare_double(double x, double y, double precision)
{
    return std::abs(x - y) < precision;
};

// Compare matrix elemnt wise up to precision
inline bool compare_mat(const arma::mat &X, const arma::mat &Y, 
    double precision)
{
    bool out = true;
