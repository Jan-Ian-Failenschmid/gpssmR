...
inline arma::mat vec2mat(const arma::vec &vector, const arma::mat &mask)
{
    arma::mat out = mask;
...
inline arma::vec mat2vec(const arma::mat &matrix, const arma::mat &mask)
{
    return matrix.elem(arma::find_nonfinite(mask));
}

inline arma::uvec reorder_row2col(arma::uword n_rows, arma::uword n_cols)
{
...
inline arma::mat diag_join(const arma::mat &A, const arma::mat &B)
{
...
// Expand grid
inline arma::mat expand_grid_2d(const arma::vec &a, const arma::vec &b)
{
...
// Jitter matrix
inline void jitter_mat(arma::mat &K, double delta)
{
    K.diag() += delta;
}

// Make matrix symmetric
inline void make_symmetric(arma::mat &X)
{
    X = 0.5 * (X + X.t());
};

// Stabailized matrix inverse and cholesky
inline void stabalized_inv(arma::mat &cov, arma::mat &cov_inv,
                           const arma::mat &cov_)
{
    try
    {
...
inline void fast_inv(arma::mat &cov_inv, const arma::mat &cov_)
{
    try
    {
        if (cov_.is_diagmat())
...
inline arma::mat identity(arma::uword n)
{
    return arma::eye(n, n);
}

// Log determinants
inline double log_det_sympd_cust(const arma::mat &X)
{
    arma::mat L = arma::chol(X, "lower");
    return 2.0 * arma::sum(arma::log(L.diag()));
}

inline double log_det_chol(const arma::mat &L)
{
    return 2.0 * arma::sum(arma::log(L.diag()));
}

// Triangular solve
inline arma::mat chol_left_solve(const arma::mat &chol, const arma::mat &v)
{
    return arma::solve(arma::trimatl(chol), v, arma::solve_opts::fast);
};

// Cholesky updates
inline void chol_rank_one_update(
    arma::mat &L,
    double c,
    arma::vec x)
{
...
inline arma::mat chol_of_sum(const arma::mat &L1, const arma::mat &L2)
{
    return arma::chol(L1 * L1.t() + L2 * L2.t(), "lower");
}

inline arma::mat chol_rank_n_update(
    arma::mat L,
    double c,
    arma::mat X)
{
...
inline void chol_rank_n_update_ip(
    arma::mat &L,
    double c,
    const arma::mat &X)
{
...
inline void add_cholesky_lower(
    arma::mat &L,
